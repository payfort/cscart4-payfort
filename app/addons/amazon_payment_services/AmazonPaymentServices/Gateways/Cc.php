...
class Cc extends Gateway {
	...
	public function __construct($title,$params){
		...
		if( $this->getConfig('show_jaywan_branding') == 'Y' || $this->getConfig('show_mada_branding') == 'Y' || $this->getConfig('show_meeza_branding') == 'Y' )
			$this->title = __('aps_mada_debit_credit_card');	
	}

	public function verifyParams($params){

		$jaywan_regex = '/^' . trim($this->getConfig('jaywan_bins')) . '/';

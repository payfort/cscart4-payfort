(function(_, $) {
	...
	setInterval(function(){
		var btnApplePay = $("button.btn_aps_applypay_checkout");
		if( btnApplePay.length ){
			if( btnApplePay.data('page') == 'product' ){
				if( !btnApplePay.hasClass('loaded_applepay') ){
					btnApplePay.addClass('loaded_applepay');
	...
	$(document).on("click",".btn_aps_applypay_checkout",function(e){
		...
		setTimeout(function(){
			initApplePayment(jelm,form);
		},50);
	});

	function initApplePayment(btn,form){
       	try{ 
			  
	       	$.ajax({ 
	       		url:fn_url("amazon_payment_services.applepay_checkout?page=product&action=add_to_cart"), 
	       		method:"POST", 
	       		data: form.serialize()+'&payment_id='+btn.data('payment_id') ,
	       		async: false, 
	       		success: function(res){
	       			if( res.success ){
						extra = res.extra;
						paymentRequest = res.data;
						btn.attr('data-params',res.data).data('params',res.data);
						btn.attr('data-extra',res.extra).data('extra',res.extra);
					} else {
						if( res.error )
				...
				error: function(){
						$.ceNotification('show',{ type: 'E', title: _.tr('error'), message: "Unable to process request, try again !" }); 
				}
			});
	       	$(".ty-ajax-loading-box,.ty-ajax-overlay").hide();
					
	       	if( !paymentRequest.countryCode ) return false;
            ...
            var snLevel = 3;
			if($.inArray('mada', paymentRequest.supportedNetworks) != -1)
				snLevel = 5;
			
			var session = new ApplePaySession(snLevel, paymentRequest );
            ...
            function getShippingOptions1( contact,form,payment_id ){
				...
				return new Promise(
		            function(resolve, reject) {
						...
						_fdata = _fdata+'selected_payment_method='+payment_id+'&payment_id='+payment_id+'&shipping_id=0&security_hash='+_.security_hash;

		                $.ajax({ 
		                	url:fn_url("amazon_payment_services.applepay_checkout?page=product&action=get_shippings"), 
		                	data: _fdata, 
		                	method: "POST", 
		                	success: function(res){
			...
			function processAspProductOrder(btn,form,aptoken,ap_contact){
				...
				_fdata = _fdata+'selected_payment_method='+payment_id+'&payment_id='+payment_id+'&shipping_id='+slcSIdentifier.replace('si_','')+'&security_hash='+_.security_hash;
				...
				$.ajax({ url:fn_url("amazon_payment_services.applepay_checkout?page=product&action=place_order"), method:"POST", data:_fdata , success: function(res){
